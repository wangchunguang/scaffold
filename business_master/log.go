...
import (
	"business_master/func_go"
	"business_master/func_string"
	"business_master/func_time"
	"fmt"
	"net/http"
	"os"
	"path"
...
func (r *FileLogger) Write(str string) {
	...
	if r.MaxSize > 0 && newsize >= r.MaxSize {
		r.file.Close()
		r.file = nil
		newpath := r.dirname + "/" + r.filename + fmt.Sprintf("_%v", func_time.Date()) + r.extname
...
func (r *Log) initFileLogger(f *FileLogger) *FileLogger {
	if f.file == nil {
		...
		f.Path = func_string.StrReplace(f.Path, "\\", "/")
		...
		if err == nil {
			...
			if f.Timeout > 0 {
				func_time.SetTimeout(f.Timeout*1000, func(...interface{}) int {
					defer func() { recover() }()
					r.ctimeout <- f
					return 0
				})
...
func (r *Log) start() {
	func_go.GoForLog(func(cstop chan struct{}) {
		var i int32
		for !r.IsStop() {
			select {
			case s, ok := <-r.cwrite:
			...
			case c, ok := <-r.ctimeout:
				if ok {
					c.file.Close()
					c.file = nil
					newpath := c.dirname + "/" + c.filename + fmt.Sprintf("_%v", func_time.Date()) + c.extname
					...
					if c.OnTimeout != nil {
						...
						if nc > 0 {
							func_time.SetTimeout(nc*1000, func(...interface{}) int {
								defer func() { recover() }()
								r.ctimeout <- c
								return 0
							})
...
func (r *Log) write(levstr string, v ...interface{}) {
	...
	if ok {
		...
		prefix = fmt.Sprintf("[%s][%s][%s:%d]:", levstr, func_time.Date(), (string)(([]byte(file))[i:]), line)
	}
	if len(v) > 1 {
